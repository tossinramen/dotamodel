...
def fetch_test_dataset(target_match_count=10):
    ...
    # Phase 1: Collect a quick batch of historical Match IDs
    ...
    batch = res.json()
    # Take just the first 10 match IDs for the test
    ...
    # Phase 2: Fetch detailed roster, players, and heroes for each ID
    ...
    for i, match_id in enumerate(match_ids_to_fetch):
        ...
        if res.status_code == 200:
            ...
            # Extract player and hero info for all 10 participants
            for player in data.get('players', []):
                slot = player.get('player_slot', 0)
                side = "radiant" if slot < 128 else "dire"
                idx = (slot if slot < 128 else slot - 128) + 1 # Convert to index 1-5
        ...
        time.sleep(1.1) # Safe rate-limiting pause between calls
